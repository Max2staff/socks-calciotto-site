...
// Le partite di una squadra viste dalla sua prospettiva (gf/gs, casa, esito).
// Senza il secondo argomento sono le nostre: lo passano le viste che guardano un'altra squadra.
export function partiteSquadra(torneo, idSquadra = torneo.squadra.id) {
  return torneo.partite
    .filter(p => p.casaId === idSquadra || p.ospiteId === idSquadra)
    .map(p => {
      const casa = p.casaId === idSquadra;
