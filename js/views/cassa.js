...
const TIPI = { pagamento: 'Pagamento', multa: 'Multa', spesa: 'Spesa', rettifica: 'Rettifica', entrata: 'Entrata extra' };
...
function renderTabellone(c, giocatori, descPartita) {
  ...
  const righe = giocatori.map(g => {
    ...
    const statoChip = g.mister ? '<span class="chip chip-neutro">Mister</span>'
      : g.saldoCent < 0
